...
export const Hero: React.FC<HeroProps> = ({ onNavigate }) => {
  ...
  return (
    <section
      id="hero"
      className="relative min-h-[88vh] flex flex-col justify-center pt-12 sm:pt-16 pb-16 px-6 sm:px-10 lg:px-16 max-w-6xl mx-auto section-hero-sky rounded-3xl sm:rounded-[36px] overflow-hidden border border-[#E8A9C4]/60"
    >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-8 xl:gap-12 items-center relative z-10">
        ...
        <div className="lg:col-span-7 space-y-6">
          ...
          <div className="space-y-2">
            <h1 className="text-4xl sm:text-6xl lg:text-5xl xl:text-6xl 2xl:text-7xl font-serif-dreamy font-bold tracking-tight text-[#1A1924] leading-[1.06]">
              {name}
            </h1>
            <p className="text-base sm:text-lg lg:text-lg font-sans-humanist text-[#473B52] font-medium tracking-wide max-w-xl">
          ...
          <div className="pt-2 flex flex-wrap items-center gap-4">
            <button
              ...
              className="px-6 sm:px-7 py-3 sm:py-3.5 rounded-full text-xs sm:text-sm font-semibold tracking-wide sky-accent-btn cursor-pointer"
            >
              Explore Projects
            </button>
            <button
              onClick={() => onNavigate('contact')}
              className="px-6 sm:px-7 py-3 sm:py-3.5 rounded-full text-xs sm:text-sm font-semibold tracking-wide dreamy-card hover:border-[#C266A7] text-[#22172E] transition-all duration-300 cursor-pointer shadow-sm"
            >
              Get in Touch
            </button>
          </div>
        ...
        {/* Right Column: Visually Distinct Stat Cards */}
        <div className="lg:col-span-5 space-y-4 sm:space-y-5">
          {/* Card 1: Filled Frosted Glass with Rose Highlight */}
          <div className="dreamy-card rounded-2xl sm:rounded-3xl p-5 sm:p-6 lg:p-6 xl:p-7 flex flex-col justify-between transition-all duration-300 group border border-[#E8A9C4]/70">
            <div className="flex items-center justify-between">
              <span className="text-3xl sm:text-4xl xl:text-5xl font-serif-dreamy font-bold text-[#1F1929] group-hover:text-[#8C3E75] transition-colors">
                {quickStats[0].value}
              </span>
              <span className="w-3 h-3 rounded-full sky-bullet" />
            </div>
            <div className="text-sm sm:text-base font-semibold text-[#2B1F38] mt-3">
              {quickStats[0].label}
            </div>
          ...
          <div className="dreamy-card-outlined rounded-2xl sm:rounded-3xl rounded-br-[36px] sm:rounded-br-[40px] p-5 sm:p-6 lg:p-6 xl:p-7 flex flex-col justify-between transition-all duration-300 group">
            <div className="flex items-center justify-between">
              <span className="text-3xl sm:text-4xl xl:text-5xl font-serif-dreamy font-bold text-[#1F1929] group-hover:text-[#8C3E75] transition-colors">
                {quickStats[1].value}
              </span>
